...
// In standard xv6, read() blocks execution. To implement non-blocking "q to
// quit", you must either use the `select` system call (if you have implemented
// it in your kernel) or use O_NONBLOCK flags. Here is a placeholder sleep loop
// you can modify.
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
      ...
      if (strcmp(argv[i], "cpu") == 0)
        ...
        printf("[INVALID VALUE FOR --sort]: '%s' (Valid: cpu, mem, time, pid, "
               "ppid)\n",
               argv[i]);
        flag = -97;
      }
...
int main(int argc, char *argv[]) {
  ...
  for (int iter = 0; iter < config.max_iterations || config.max_iterations == 0;
    ...
    printf("==================================================================="
           "=====================================\n");
    printf(" System Uptime: %d.%ds | Total Processes: %d | Running Processes: "
           "%d\n",
           (up_ticks / 10), up_ticks % 10, n, running_count);

    if (mem_stats_ok) {
      long total_disk =
          umemstat.read_disk_counter + umemstat.write_disk_counter;
      long total_cache =
          umemstat.read_cache_counter + umemstat.write_cache_counter;
      ...
      print_percentage(umemstat.read_cache_hit_counter,
                       umemstat.read_cache_counter);
      printf(" (Hits: %d)\n", (int)umemstat.read_cache_hit_counter);
    } else {
      printf(" [MEMSTAT ERROR]: Could not fetch memory, cache, and disk stats "
             "from kernel!\n");
    }
    ...
    char title_buf[16];
    strcpy(title_buf, (config.cpu_opt == 'p'
                           ? "CPU(%)"
                           : (config.cpu_opt == 's' ? "CPU(s)" : "CPU(tick)")));
